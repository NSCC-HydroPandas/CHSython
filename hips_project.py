...
try:
    from chs_utils import hips_utils
except ImportError:
    import hips_utils
...
class HIPSProject(HIPSObject):
    ...
    def __init__(self, path_to_project):
        ...
        result = self.select_query(columns='RELEASE_VERSION',
                                   table='hipsProjectVersion'
                                   )
    ...
    def get_line_from_path(self, line_path, verify=False):
        ...
        # check if it's a URI and convert to a line path instead
    ...
    def get_vessels(self):
        ...
        if self.get_attribute('Version') == 1:
            for line in self.get_lines():
                ...
                if vess_path not in (item[1] for item in vess):
    ...
    def get_ids_by_type(self, type, related_id=None, search=''):
        ...
        if self.get_attribute('Version') == 1:
            ...
            if 'Line' in type:
                criteria.add_query(
                    """Line.concreteObjectId NOT IN 
                    (SELECT objectState.concreteObjectId 
                    from objectState 
                    WHERE deleted=1)""")
                if search != '':
    ...
    @staticmethod
    def camel_to_string(name):
        """Converts a camelCase string to Title Case"""
        return re.sub(
            r'((?<=[a-z])[A-Z]|(?<!\A)[A-Z](?=[a-z]))', r' \1', name).title()

    @staticmethod
    def string_to_camel(name):
